...
function canonicalSpreadAnchor(page: number, visiblePages: number[]): number {

  const { left } = resolveSpreadAnchorPages(page, visiblePages)
...
function appendSpreadPdfPages(
  ...
  for (const p of spreadPdfPagesForAnchor(anchor, visiblePages)) {
...
function collectAdjacentSpreadAnchors(args: {
  ...
  for (let i = 0; i < args.count; i++) {
    ...
    anchors.push(canonicalSpreadAnchor(next, args.visiblePages))
...
export function splitReaderPrefetchPages(
  ...
  const currentAnchor = canonicalSpreadAnchor(clampedAnchor, visiblePages)
  ...
  for (const anchor of orderedAnchors) {
    ...
    appendSpreadPdfPages(immediate, immediateSet, anchor, visiblePages)
